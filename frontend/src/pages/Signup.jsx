...
export default function Signup() {
  ...
  const handleSubmit = async () => {
    try{
      toast.loading("Sigining Up",{id:"Signup"});
  ...
  return (
    <Box width={"100%"} height={"100%"} display="flex" flex={1}>
      ...
      <Box display={'flex'} flex={{ xs: 0.5, md: 0.5 }} justifyContent={"center"} alignItems={'center'} padding={2} ml={40} mt={1}>
        ...
        >
          <div style={{ display: "flex", justifyContent: 'center', margin: '13px 0px' }}><h1 style={{ margin: '0px' }}>Signup</h1></div>
          <TextInput placeholder='Enter Your Name' type='text' onChange={(e) => setName(e.target.value)}></TextInput>
          ...
          <Button onClick={handleSubmit} text='Signup'></Button>
        </div>
      </Box>
    </Box>
  )
